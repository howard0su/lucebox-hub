...
// MoE entry points (defined in test_dflash_moe.cpp)
int run_moe_autoregressive(const char * target_path, const char * prompt_path,
                           int n_gen, const char * out_path,
                           ggml_backend_t backend, dflash27b::TargetWeights & w);
int run_moe_dflash(const char * target_path, const char * draft_path,
                   const char * prompt_path, int n_gen, const char * out_path,
                   int budget, ggml_backend_t backend, ggml_backend_t draft_backend,
                   dflash27b::TargetWeights & w, dflash27b::DraftWeights & dw,
                   bool ddtree_mode, int ddtree_budget, float ddtree_temp,
...
int main(int argc, char ** argv) {
    ...
    int   moe_budget = 16;          // MoE verify budget (1 = pure autoregressive)
    ...
    // For MoE with budget==1 (pure autoregressive), skip draft model entirely
    if (w.is_moe && moe_budget == 1) {
        int rc = run_moe_autoregressive(target_path, prompt_path, n_gen, out_path,
                                        target_backend, w);
    ...
    // MoE with spec-decode (budget > 1)
    if (w.is_moe) {
        int rc = run_moe_dflash(target_path, draft_path, prompt_path, n_gen, out_path,
                                moe_budget, target_backend, draft_backend, w, dw,
